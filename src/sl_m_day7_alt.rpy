label slavyana_mod__day7_alt:
  ...
  #Рут Лены или одиночки
    ...
    "Получив порцию, я быстро села за столик, но вскоре ко мне подсела Женя."
    ...
    "Но прежде чем идти в столовую, думаю стоит попытаться выловить Шурика."
    stop sound fadeout 2
    "Зайду к кибернетикам, всё равно по пути"
    ...
    scene bg int_dining_hall_people_day with dissolve
    window show
    "Обед, равно как и завтрак, не представлял из себя ничего необычного."
    "Уже после того как я поела, я ещё некоторое время сидела, рассматривая заходящих и уходящих пионеров."
    ...
    "..."
    "Когда было готово, я перечитала. {w}И обнаружила, что почерки были очень похожи."
    "Даже не так... {w}Они были идентичными!"
    th "Я что, настолько сильно перекопировала, что даже почерк похожий? Да нет, бред какой-то!"
...
label slavyana_mod__day7_ending:
    ...
    if current_route == Routes.SoloDV:
        ...
        if current_route == Routes.SoloUN:
            ...
            "Как только вернулся Семён, стоянка вдруг начала быстро наполняться пионерами."
        else:
