...
label slavyana_mod__day5:
  ...
  if go_to_sh:
    ...
    #*Иллюстрация с неко-чан с помехами*
...
#*Фон внутри домика Слави*
  ...
  "Забежав домой, я уже достала форму и корзинки, как вдруг в дверь кто-то постучал."
  sl "Войдите."
...
#Выбор(Пойти с Семёном / Я и сама могу)

#Если выбрали «пойти с Семёном» +1:
...
#Выбор(Переодеться до уборки костра/После уборки костра)

#Если выбрали «переодеться до уборки костра»
...
#Если выбрали «после уборки костра»
#[то же самое, но уже ПОСЛЕ уборки костра, я напомню]

#конец трека
#Фон тёмный экран
  "…"
  
#Если выбрала "переодеться до уборки костра" (в выборе когда переодеться для пляжа, перед походом на поляну)
#*Фон тропинка в лесу*
...
#В этом монологе Лены, прямо посреди фраз меняются спрайты или добавляется музыка, обрати внимание!
  un "Ну вот в смысле твоего к него отношения. {w} !*страшный звук*! {i}Нравится{/i} ли он тебе?"
  sl "Лен, это личные вопросы…"
  un "А я так не думаю. {w} *спрайт со стрёмной улыбкой* Я же вижу как ты на него смотришь и не стесняешься."
  
#Выбор без очков, от которого зависит гуд/бэд концовка если на момент выбора, очков <4, то автоматом выбирается «Соврать» (Соврать/Сказать правду)

#Если выбрали «Соврать»(автоматом выход на бэд концовку, а при втором прохождении альтернативную концовку(третью)):
  sl "Никак я на него не смотрю. С чего ты взяла?"
  un "Ну да, ты ещё врать будешь."
  sl "Нет."
#*Спрайт с орущей Леной*
...
#Если выбрали «Сказать правду»(открывает гуд концовку, если после разговора с ОД на площади очков >=9):
  sl "Я ещё точно не знаю, не уверена, но… наверное. А почему ты интересуешься?"
#спрайт стесняющейся Лены
  un "Да нет, просто…"
...
#На эту фразу тоже обрати внимание(которая ниже)
  un "Заметно, что Семён отвечает тебе взаимностью, поэтому не буду вам мешать. Мы же всё-таки подруги. {w} !*Страшный звук* *Спрайт со стрёмной улыбкой*! {i}Пока{/i}."
